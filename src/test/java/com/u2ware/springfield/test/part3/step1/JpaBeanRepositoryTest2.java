...
public class JpaBeanRepositoryTest2 extends AbstractRootContextTest{
	...
	public void testAll() throws Exception{
		...
		Page<JpaBean> page = jpaBeanComponent.findAll(pageable);
		for(JpaBean p : page){
			logger.warn(""+p);
		}
		...
		page = jpaBeanComponent.findAll(query, pageable);
		for(JpaBean p : page){
			logger.warn(""+p);
		}
		...
		long result = jpaBeanComponent.deleteAll(query);
		logger.warn(""+result);
		...
		result = jpaBeanComponent.deleteAll(query);
